...
# 워커 프로세스 타임아웃(초). 구모델 기준 Render 무료 플랜에서 계산이 1분 가까이
# 걸리는 걸 확인했었다(로컬 대비 5~8배). 2026-09 모델 교체(열화 반영 lifecycle
# LCOH) 이후 로컬 계산시간이 22초로 늘어서(Case 3 최적화 버그 수정 후 기준),
# 같은 배율이면 Render에서 110~176초까지 걸릴 수 있다고 보고 여유를 더 둔다.
# 실제 Render 배포 후 꼭 재확인할 것.
